...
def traduzir_payload(data):
    data['basics']['label_en'] = traduzir_texto(data['basics'].get('label_pt', ''))
    
    if 'summary' in data and 'pt' in data['summary']:
        data['summary']['en'] = [traduzir_texto(p) for p in data['summary']['pt']]
        
    for exp in data.get('experience', []):
        exp['position_en'] = traduzir_texto(exp.get('position_pt', ''))
        exp['highlights_en'] = [traduzir_texto(h) for h in exp.get('highlights_pt', [])]
